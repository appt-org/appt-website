---
locale: nl
slug: /wcag/succescriterium-2-5-4
contentful_id: EYRl5ZUoQPHOLIqj2tLqN
icon: /img/light/2.5.4.svg
title: Succescriterium 2.5.4 - Bewegingsactivering - Niveau A
description: Zorg dat er een alternatief is voor acties door beweging én maak het mogelijk om ze uit te schakelen. Voor gebruikers met beperkte handfunctie is schudden vaak niet mogelijk. Bij gebruikers met spasmen kunnen de acties juist ongewenst worden geactiveerd.
authors: [janjaap]
hide_title: true
date: 2022-06-16
last_update:
  date: 2024-12-18
---

<Hero
  title="Bewegingsactivering"
  image="/img/light/2.5.4.svg"
  alt="WCAG 2.5.4 icon"
  prefix="Succescriterium 2.5.4"
  suffix="Niveau A"
/>

Zorg dat er een alternatief is voor acties door beweging én maak het mogelijk om ze uit te schakelen. Voor gebruikers met beperkte handfunctie is schudden vaak niet mogelijk. Bij gebruikers met spasmen kunnen de acties juist ongewenst worden geactiveerd.

## Impact

- Voor gebruikers met (tijdelijk) beperkte handfunctie is schudden vaak niet mogelijk. Zorg ervoor dat deze functies ook op een andere manier geactiveerd kunnen worden.
- Een bewegende rolstoel of spasmen kunnen onbedoeld functies activeren. Zorg dat deze functies uitgezet kunnen worden.

## Controleren

“*Is er een alternatieve manier beschikbaar voor functies die door schudden geactiveerd worden?*“

Dit kan getest worden zonder hulpmiddelen.

## Oplossing

### Bied alternatieven voor beweging geactiveerde functies

<CodeSample id="input-motion" locale="nl"/>

## Bronnen

- [WCAG 2.2 - Succescriterium 2.5.4 - Bewegingsactivering](https://www.w3.org/Translations/WCAG22-nl/#bewegingsactivering)
