---
locale: en
slug: /wcag/success-criterion-1-4-12
contentful_id: 1en4RNLR6cRofp4AmStBpf
icon: /img/light/1.4.12.svg
title: Success Criterion 1.4.12 - Text Spacing - Level AA
description: Ensure there is enough space between paragraphs, letters and words. People with dyslexia can read faster as a result. People who are visually impaired can also read the text more easily. White space can also help people with a cognitive impairment to distinguish parts from each other.
authors: [janjaap]
hide_title: true
date: 2022-06-16
last_update:
  date: 2024-12-18
---

<Hero
  title="Text Spacing"
  image="/img/light/1.4.12.svg"
  alt="WCAG 1.4.12 icon"
  prefix="Success Criterion 1.4.12"
  suffix="Level AA"
/>

Ensure there is enough space between paragraphs, letters and words. People with dyslexia can read faster as a result. People who are visually impaired can also read the text more easily. White space can also help people with a cognitive impairment to distinguish parts from each other.

## Impact

- People with dyslexia can read faster if sufficient distance between texts is used.
- People who have poor vision can read more easily if there is sufficient distance between text, words and paragraphs.

## Check

*“Is there enough distance between letters, lines and paragraphs?”*

This can be tested visually, no assistive technologies are needed.

## Solution

### Adjust text spacing

<CodeSample id="text-spacing" locale="en"/>

### Support bold text

<CodeSample id="text-bold" locale="en"/>

## Resources

- [WCAG 2.2 - Success Criterion 1.4.12 - Text Spacing](https://www.w3.org/TR/WCAG22/#text-spacing)
